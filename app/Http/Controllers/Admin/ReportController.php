...
class ReportController extends Controller
{
	...
	public function exportRevenue(Request $request)
	{
		...
		// Get revenues based on the selected date range
		$revenues = Revenue::selectRaw('DATE(date) as date, COUNT(*) as num_of_orders, SUM(gross_revenue) as gross_revenue, SUM(taxes_amount) as taxes_amount, SUM(shipping_amount) as shipping_amount, SUM(net_revenue) as net_revenue')
			->whereBetween('date', [$start, $end])
			->groupBy('date')
			->get();

		// Export data based on the selected export type
		if ($exportType == 'pdf') {
			$pdf = PDF::loadView('admin.reports.exports.revenue-pdf', compact('revenues', 'start', 'end'));
			return $pdf->download('revenues.pdf');
		} else if ($exportType == 'xlsx') {
			$data = [];
			foreach ($revenues as $revenue) {
				$data[] = [
					'Date' => $revenue->date,
					'Orders' => $revenue->num_of_orders,
					'Gross Revenue' => $revenue->gross_revenue,
					'Taxes' => $revenue->taxes_amount,
					'Shipping' => $revenue->shipping_amount,
					'Net Revenue' => $revenue->net_revenue,
				];
			}
			return Excel::download(new RevenueExport($data), 'revenues.xlsx');
		}
	}
